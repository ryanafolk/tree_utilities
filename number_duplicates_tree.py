...
# Run like:
# ./number_duplicates_tree.py intree.tre outtree.tre

# If more than one duplicate, you will get node labels like _2_2_2
# This can be cleaned up by hand
	...
	for i in duplicate_list:
		for node in sp_tree.leaf_node_iter(): # Iterate over leaves (can't iterate over taxa per above)
			if taxonremoved == 1: # Switch variable we use to prune only the first duplicate
				...
				#print(i)
				# node.edge.tail_node.remove_child(node, suppress_unifurcations = True)
				node.label = "_".join([i, "2"]) # Suppress unifurcations, which can arise if leaves are trimmed but leave behind orphan nodes
